...
const Dashboard = () => {
  ...
  const fetchBills = async () => {
    try {
      ...
      setSelectedBill(res.data.bills?.[0] || null);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  /* Charts */
  const lineData = {
    labels: bills.slice().reverse().map(b =>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
            ...
            <div>
                <p className="text-sm text-slate-500 capitalize">{selectedBill?.type || "No Data"}</p>
                <p className="text-3xl font-bold text-slate-900">₹{selectedBill?.amount.toLocaleString() || 0}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleUpload} className="grid md:grid-cols-4 gap-5">
            <div className="flex flex-col gap-1">
                ...
                <select 
                    ...
                    className="border border-slate-300 rounded-lg px-3 py-2.5 bg-slate-50 focus:ring-2 focus:ring-indigo-500 outline-none"
                >
            ...
            <div className="flex flex-col gap-1">
                 ...
                 <input 
                    ...
                    className="border border-slate-300 rounded-lg px-3 py-2 bg-slate-50 text-sm file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-indigo-100 file:text-indigo-700 hover:file:bg-indigo-200"
                />
            </div>
            ...
            <div className="md:col-span-4 flex flex-col gap-1">
                 ...
                 <textarea
                    className="border border-slate-300 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none"
                    placeholder="e.g. Explain like I'm 10 / Hindi"
        ...
        {selectedBill && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 space-y-6">
            ...
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-100">
                <p className="text-slate-700 leading-relaxed text-lg">{analysis?.explanation || "Analysis pending..."}</p>
            </div>

            <div className="grid md:grid-cols-2 gap-6">
                {analysis?.scamAlerts?.length > 0 && (
                <div className="bg-rose-50 border border-rose-100 rounded-xl p-5">
                    ...
                    <ul className="list-disc ml-5 space-y-1 text-rose-800">
                        {analysis.scamAlerts.map((a, i) => <li key={i}>{a.message}</li>)}
                    </ul>
                </div>
                )}
                ...
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-5">
                    ...
                    <ul className="list-disc ml-5 space-y-1 text-emerald-800">
                        {analysis.suggestions.map((s, i) => <li key={i}>{s.message}</li>)}
                    </ul>
                </div>
                )}
            </div>

            {analysis?.optionalCharges?.length > 0 && (
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-5">
                <h3 className="font-bold text-amber-700 mb-3">Optional Charges Found</h3>
                <div className="flex flex-wrap gap-2">
                  {analysis.optionalCharges.map((c, i) => (
        ...
        {/* 4. CHARTS - ONLY SHOW IF DATA EXISTS */}
        {bills.length > 0 ? (
            <div className="grid md:grid-cols-2 gap-6">
              ...
              >
                <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-full ${
                        ...
                        bill.type === 'hospital' ? 'bg-rose-100 text-rose-600' :
                        'bg-indigo-100 text-indigo-600'
                    }`}>
                        {bill.type === 'electricity' ? <Zap className="w-5 h-5"/> : 
                        bill.type === 'hospital' ? <Activity className="w-5 h-5"/> : 
